    ...
    saved_changes.each do |field, (old_value, new_value)|
      ...
      when 'assigned_to_id'
        if new_value.present?
          assigned_user = User.find_by(id: new_value)
          activities.create!(
            user: current_user,
            action: 'assigned',
            field_name: field,
            old_value: old_value.present? ? User.find_by(id: old_value)&.name : nil,
            new_value: assigned_user&.name
          )
        else
          old_user = User.find_by(id: old_value)
          activities.create!(
            user: current_user,
            action: 'unassigned',
            field_name: field,
            old_value: old_user&.name,
            new_value: nil
          )
        end
      when 'due_date'
